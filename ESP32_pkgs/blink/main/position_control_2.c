...
#define EXO_ESP_ID "RIGHT_FLEXION"
...
{
    return angle_radians / 360 * 4096 * 2 * REDUCTION_RATIO;
}

int get_angle_from_counter(int32_t counter_value)
{
    return (counter_value /(REDUCTION_RATIO *2 * 4096.0)) * 360 ;
}
...
void update_current_values(){
    ...
    printf(" %f", test_feedback_payload.joint_angle);
    printf(" %f", current_velocity);
}

void duty_update(float new_duty, int error_flag){
    if(error_flag ){
        printf("error flag");
        if(new_duty > 0){
            new_duty += 49;
    ...
    if(new_duty > 250)new_duty = 250;
    else if(new_duty < -250)new_duty = -250;
...
void position_control(){
    ...
    float d_error = position_error - past_error; 
    float new_duty = position_pid.p * position_error + position_pid.i * position_error_i + position_pid.d *d_error;
    past_error = position_error;
    if(position_error > 0.5 || position_error < -0.5 ){
        duty_update(new_duty, 1);
    }
    else{
        position_error_i = 0;
        duty_update(new_duty, 0);
...
void feedforward_velocity_control(float reference_velocity){
    static float velocity_error_i = 0;
    //velocity control
    printf(" %f", reference_velocity);
    float velocity_error = reference_velocity - test_feedback_payload.velocity;
...
void feedback_feedforward_control(){
    static float position_error_i = 0;
    float position_error = received_payload.goal_position - test_feedback_payload.joint_angle;
    float reference_velocity = received_payload.goal_velocity + position_pid.p*position_error + position_pid.i*position_error_i;
    position_error_i += position_error;
    
    //limiting the position error contribution depending upon the goal velocity 
    if(position_pid.i*position_error_i > 0.3*received_payload.goal_velocity){
        position_error_i = 0.3*received_payload.goal_velocity/position_pid.i;
    }
    feedforward_velocity_control(reference_velocity);
}
...
void update_bldc_speed()
{
    ...
    motor_speed = hall_counts / (4096 * 4 * 0.2); // the update speed in 0.2
      if(received_payload.mode == 0) duty = 0;
      else if(received_payload.mode == 1) position_control();
      else if(received_payload.mode == 2) feedforward_velocity_control(received_payload.goal_velocity);
      else if(received_payload.mode == 3) feedback_feedforward_control();
      else if(received_payload.mode == 4) effort_control();
      else if(received_payload.mode == 5) rotation_direction = true;
      else if(received_payload.mode == 6) rotation_direction = false;
      if(test_feedback_payload.velocity == 0){
